...
export const RegistrationComponent = () => {
  ...
  const [registrationFormHook, bindRegistrationFormHook, setRegistrationFormHook] = useFormField(RegistrationLabels.discriminatorRegistrationName, {});
      ...
      <div>
        <div className={'center-div'}>
          <div>
            ...
            <div className={'center-content'}>
              <h1 className={'color-white-text'}>{RegistrationLabels.welcomeYACO}</h1>
            </div>
            <div className={'center-content'}>
              <h4 className={'color-white-text'}>{RegistrationLabels.accountAlready}</h4>
            </div>
            <div className={'center-content'}>
              <button className={'button-style-login'}
                      onClick={() => addToHistory(history, LOGIN_PATH)}>{RegistrationLabels.login}
              </button>
            </div>
          </div>
        </div>
        <div className={'background-form'}>
          <div>
            {REGISTRATION_FORM_DESCRIPTOR.map((descriptor, index) =>
              <div key={`${RegistrationLabels.idFieldRegistrationName}-${descriptor.properties.name}-${index}`}>
                <FieldFactoryComponent {...descriptor} bind={bindRegistrationFormHook}/>
              </div>)}
          </div>
